...
class GBDTCatBoost:
    def __init__(self, task='regression', depth=6, learning_rate=0.1, l2_leaf_reg=0., max_bin=254):
        self.task = task
        self.depth = depth
...
class GBDTLGBM:
    def __init__(self, task='regression', learning_rate=0.1, num_leaves=31, max_bin=255,
    ...
    def get_metrics(self):
        ...
        for metric_name in d[keys[0]]:
            perf = [d[key][metric_name] for key in keys]
            if metric_name == self.parameters['objective']:
                metrics['loss'] = list(zip(*perf))
            else:
                metrics[metric_name] = list(zip(*perf))
